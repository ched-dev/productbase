...
migrate((app) => {
  ...
  console.log('ProductBase: Updating Files/Backups Settings |',
    `S3_ACCESS_KEY_ID=${Boolean(S3_ACCESS_KEY_ID)}`,
    `S3_SECRET_ACCESS_KEY=${Boolean(S3_SECRET_ACCESS_KEY)}`,
  ...
  if (S3_ACCESS_KEY_ID && S3_SECRET_ACCESS_KEY && S3_ENDPOINT && S3_REGION && S3_FILES_BUCKET) {
    // files
    settings.s3.enabled = true;
    settings.s3.accessKey = S3_ACCESS_KEY_ID;
    settings.s3.secret = S3_SECRET_ACCESS_KEY;
    ...
    settings.backups.s3.accessKey = S3_ACCESS_KEY_ID;
    settings.backups.s3.secret = S3_SECRET_ACCESS_KEY;
