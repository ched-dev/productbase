...
migrate((app) => {
  ...
  if (S3_FILES_ACCESS_KEY_ID && S3_FILES_SECRET_ACCESS_KEY && S3_FILES_ENDPOINT && S3_FILES_REGION && S3_FILES_BUCKET) {
    ...
    console.log('ProductBase: Updating Files Bucket Settings |',
      `S3_FILES_ACCESS_KEY_ID=${Boolean(settings.s3.accessKey)}`,
      `S3_FILES_SECRET_ACCESS_KEY=${Boolean(settings.s3.secret)}`,
      `S3_FILES_ENDPOINT=${settings.s3.endpoint}`,
      `S3_FILES_REGION=${settings.s3.region}`,
      `S3_FILES_BUCKET=${settings.s3.bucket}`
    )
  }
  if (S3_BACKUPS_ACCESS_KEY_ID && S3_BACKUPS_SECRET_ACCESS_KEY && S3_BACKUPS_ENDPOINT && S3_BACKUPS_REGION && S3_BACKUPS_BUCKET) {
    ...
    console.log('ProductBase: Updating Backups Bucket Settings |',
      `S3_BACKUPS_ACCESS_KEY_ID=${Boolean(settings.backups.s3.accessKey)}`,
      `S3_BACKUPS_SECRET_ACCESS_KEY=${Boolean(settings.backups.s3.secret)}`,
      `S3_BACKUPS_ENDPOINT=${settings.backups.s3.endpoint}`,
      `S3_BACKUPS_REGION=${settings.backups.s3.region}`,
      `S3_BACKUPS_BUCKET=${settings.backups.s3.bucket}`
    )
  }
