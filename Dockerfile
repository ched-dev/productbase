# Stage 1: Build the static frontend files with Deno & SvelteKit
FROM node:20-alpine AS frontend-builder
WORKDIR /app
COPY ./frontend/version.json ./frontend/version.json
COPY ./frontend ./
ARG PUBLIC_PRODUCTION=$PUBLIC_PRODUCTION
ARG PUBLIC_FRONTEND_URL=$PUBLIC_FRONTEND_URL
ARG PUBLIC_PB_API_URL=$PUBLIC_PB_API_URL
ARG PUBLIC_STRIPE_BUTTON_ID=$PUBLIC_STRIPE_BUTTON_ID
ARG PUBLIC_STRIPE_PUBLISHABLE_KEY=$PUBLIC_STRIPE_PUBLISHABLE_KEY
...
ARG PB_APPLICATION_NAME=$PB_APPLICATION_NAME
ARG PB_API_URL=$PB_API_URL
